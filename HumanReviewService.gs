...
var HumanReviewService = (function () {
  ...
  function createDraft(useCase, content, options) {
    try {
      ...
      var record = {
        ...
        ownerId: String(options.ownerId || '').slice(0, 120)
      };
      write_(record);
  ...
  function decorateResult(useCase, result, content, options) {
    var draft = createDraft(useCase, content, options || {});
...
function requireToolDebateReviewPrincipal_(token) {
  return requireAuthenticatedPrincipal_(token || '');
}

function assertToolDebateDraftOwner_(id, principal) {
  var draft = HumanReviewService.get(id);
  var owner = String(principal.username || principal.userId || '');
  if (!draft.ownerId || String(draft.ownerId) !== owner) {
    throw new Error('Rascunho pertence a outro estudante.');
  }
  return draft;
}

function createAiReviewDraft(token, useCase, content, options) {
  var principal = requireToolDebateReviewPrincipal_(token);
  options = options || {};
  options.ownerId = String(principal.username || principal.userId || '');
  return HumanReviewService.createDraft(useCase, content, options);
}
function editAiReviewDraft(token, id, content) {
  var principal = requireToolDebateReviewPrincipal_(token);
  assertToolDebateDraftOwner_(id, principal);
  return HumanReviewService.editDraft(id, content, principal.username || principal.userId);
}
function rejectAiReviewDraft(token, id, reason) {
  var principal = requireToolDebateReviewPrincipal_(token);
  assertToolDebateDraftOwner_(id, principal);
  return HumanReviewService.reject(id, reason, principal.username || principal.userId);
}
function approveAiReviewDraft(token, id, version) {
  var principal = requireToolDebateReviewPrincipal_(token);
  assertToolDebateDraftOwner_(id, principal);
  return HumanReviewService.approve(id, version, principal.username || principal.userId);
}
function getAiReviewDraft(token, id) {
  try {
    var principal = requireToolDebateReviewPrincipal_(token);
    assertToolDebateDraftOwner_(id, principal);
...
function assertAiReviewApproved(token, id, version) {
  var principal = requireToolDebateReviewPrincipal_(token);
  assertToolDebateDraftOwner_(id, principal);
