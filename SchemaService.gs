...
// USERS aponta para a aba REAL de login deste projeto ("Usuarios"), com os
// cabecalhos que Auth.gs espera. Antes apontava para uma aba inglesa "Users"
// inexistente na planilha — o ensureAllSheets acabava criando uma segunda aba
// de usuarios (a planilha viva chegou a ter "Usuarios" E "Users"). A coluna
// "PasswordHash" fica reservada apenas por compatibilidade de schema; a frota
// opera em TEXTO PLANO e a migracao Manutencao.migrarSenhasParaHash foi NEUTRALIZADA
// permanentemente (retorna erro, corpo removido, hashSenha_ retorna texto plano).
var PROJECT_SCHEMA_DEFINITIONS = {"AUDIT_LOGS":{"entity":"AUDIT_LOGS","sheetName":"Audit_Logs","headers":["ID","Timestamp","Level","Action","Entity","RecordID","UserID","Message","Details","CreatedAt"],"required":["ID"],"identifier":"ID","source":"baseline"},"SETTINGS":{"entity":"SETTINGS","sheetName":"Settings","headers":["Key","Value","Description","Scope","UpdatedAt","UpdatedBy"],"required":["Key"],"identifier":"Key","source":"baseline"},"USERS":{"entity":"USERS","sheetName":"Usuarios","headers":["ID","Username","Password","PasswordHash","Role","Nome","Email","Status"],"required":["ID","Username"],"identifier":"ID","source":"baseline"}};

PROJECT_SCHEMA_DEFINITIONS.LEITURAS_ACERVO = {
  entity: 'LEITURAS_ACERVO',
  sheetName: 'Leituras_Acervo',
  headers: [
    'ID', 'CriadoEm', 'ConcluidoEm', 'ChaveAluno', 'Identificacao', 'Idade',
    'ArquivoId', 'ArquivoNome', 'ArquivoAtualizadoEm', 'Titulo',
    'PerguntasJson', 'RespostasJson', 'Acertos', 'TotalPerguntas',
    'PontosBase', 'BonusEscuta', 'BonusSequencia', 'PontosTotal',
    'EscutaConcluida', 'DuracaoSegundos', 'Voz', 'Velocidade',
    'Status', 'ResultadoJson'
  ],
  required: ['ID', 'ChaveAluno', 'ArquivoId'],
  identifier: 'ID',
  source: 'leitura-acervo'
};

PROJECT_SCHEMA_DEFINITIONS.PROGRESSO_LEITOR = {
  entity: 'PROGRESSO_LEITOR',
  sheetName: 'Progresso_Leitor',
  headers: [
    'ChaveAluno', 'Identificacao', 'Idade', 'Pontos', 'LeiturasConcluidas',
    'RespostasCorretas', 'TotalPerguntas', 'SequenciaAtual', 'MelhorSequencia',
    'Nivel', 'Conquistas', 'UltimaLeituraId', 'UltimaLeituraEm', 'AtualizadoEm'
  ],
  required: ['ChaveAluno'],
  identifier: 'ChaveAluno',
  source: 'leitura-acervo'
};

PROJECT_SCHEMA_DEFINITIONS.PERGUNTAS_ACERVO = {
  entity: 'PERGUNTAS_ACERVO',
  sheetName: 'Perguntas_Acervo',
  headers: [
    'ArquivoId', 'ArquivoNome', 'ArquivoAtualizadoEm', 'Titulo',
    'PerguntasJson', 'GeradoEm', 'Modelo', 'Status'
  ],
  required: ['ArquivoId'],
  identifier: 'ArquivoId',
  source: 'leitura-acervo'
};

PROJECT_SCHEMA_DEFINITIONS.DIRETRIZES = {
  entity: 'DIRETRIZES',
  sheetName: 'Diretrizes',
  headers: [
    'ID', 'Titulo', 'Universo ficcional', 'Problema narrativo', 'Paisagem DF',
    'Paisagem natural', 'Paisagem urbana', 'Foco de pesquisa', 'Ancora em referencias',
    'Personalizacao infantil', 'Interesse da crianca', 'Arranjo familiar ou social',
    'Chave emocional', 'Nuance diferenciadora', 'Motor logico e sensibilizacao',
    'ODS relacionados', 'Uso no modelo'
  ],
  required: ['ID'],
  identifier: 'ID',
  source: 'tool-debate-conto'
};

// Criacao de conto pelo aluno logado (Respondendo Questionario). Catalogada aqui
// para que o CRUD passe pelo SchemaService como as demais entidades. Os
// cabecalhos de framework abaixo sao um piso estavel; as 25 colunas das
// perguntas sao compostas em runtime a partir de obterPerguntas() (fonte unica:
// Questionario.gs), sem depender de uma aba "Diretrizes" pre-carregada
// (ver addToolDebateRespostasSchema_).
PROJECT_SCHEMA_DEFINITIONS.RESPOSTAS = {
  entity: 'RESPOSTAS',
  sheetName: 'Respostas',
  headers: [
    'Carimbo de data/hora', 'ID do Aluno', 'Identificação', 'Idade',
    'Comentário livre', 'Método de Seleção', 'ID Diretriz',
    'Status', 'Título do Conto', 'Arquivo do Conto', 'Link do Conto'
  ],
  required: ['ID do Aluno'],
  identifier: 'ID do Aluno',
  source: 'tool-debate-conto'
};

// Aba "Notas" da estrutura simplificada. Vazia e sem uso no codigo: os
// cabecalhos abaixo sao INFERIDOS (registro de notas/observacoes por aluno) e
// devem ser confirmados antes de povoar. identifier minimo para nao impor uma
// estrutura errada a uma aba ainda indefinida.
PROJECT_SCHEMA_DEFINITIONS.NOTAS = {
  entity: 'NOTAS',
  sheetName: 'Notas',
  headers: ['ID', 'ChaveAluno', 'Identificacao', 'Referencia', 'Nota', 'Comentario', 'CriadoEm'],
  required: ['ID'],
  identifier: 'ID',
  source: 'inferido'
};
// Aba de gerenciamento de sessoes de autenticacao. Criada automaticamente no primeiro login.
PROJECT_SCHEMA_DEFINITIONS.SESSOES_AUTH = {
  entity: 'SESSOES_AUTH',
  sheetName: 'SessoesAuth',
  headers: ['token', 'userId', 'username', 'role', 'expiresAt'],
  required: ['token'],
  identifier: 'token',
  source: 'auth-session-storage'
};

// Aba real lida pelo login por senha deste projeto (ver Auth/AuthLogin/UserService).
var SYNTHETIC_USERS_LOGIN_SCHEMAS = [
  { sheetName: 'Usuarios', headers: ['ID','Username','Password','Role','Nome','Email','Status'] }
];
...
var SchemaService = (function() {
  ...
  function getSpreadsheet_(options) {
    try {
      ...
      if (typeof getPlanilhaId === 'function') {
        var projectSpreadsheetId = getPlanilhaId();
        if (projectSpreadsheetId) return SpreadsheetApp.openById(projectSpreadsheetId);
      }
  ...
  // Compoe os cabecalhos completos da aba "Respostas" (criacao de conto) a partir
  // do dominio: COLUNAS_META + colunas das perguntas (obterPerguntas) + processo +
  // conto. Mantem o Questionario.gs como fonte unica e desacopla o CRUD do conto
  // da aba "Diretrizes" (ausente na estrutura simplificada). required/identifier
  // do schema estatico sao preservados pelo upsert.
  function addToolDebateRespostasSchema_(schemas) {
    try {
      try {
        if (typeof obterPerguntas !== 'function') return;
        var meta = (typeof COLUNAS_META !== 'undefined' && COLUNAS_META) ? COLUNAS_META : [];
        var processo = (typeof COLUNAS_PROCESSO !== 'undefined' && COLUNAS_PROCESSO) ? COLUNAS_PROCESSO : [];
        var conto = (typeof COLUNAS_CONTO !== 'undefined' && COLUNAS_CONTO) ? COLUNAS_CONTO : [];
        var perguntas = obterPerguntas().map(function(p) { return p.coluna; });
        var headers = meta.concat(perguntas).concat(processo).concat(conto);
        upsertRuntimeSchema_(schemas, 'RESPOSTAS', 'Respostas', headers, 'tool-debate-conto');
      } catch (error) {
        Logger.log('SchemaService Respostas compose skipped: ' + error.message);
      }
    } catch (error) {
      Logger.log("Erro em addToolDebateRespostasSchema_: " + error.message);
      throw error;
    }
  }
  ...
  function collectRuntimeSchemas_() {
    ...
    addToolDebateRespostasSchema_(schemas);
    return schemas;
  }
...
/**
 * Atualiza o cabeçalho da aba "Respostas" para incluir todas as perguntas atuais.
 * Execute esta função manualmente após adicionar ou remover perguntas do questionário.
 * 
 * @return {Object} Resultado da atualização com detalhes das colunas
 */
function atualizarCabecalhoRespostas() {
  try {
    try {
      Logger.log('Iniciando atualização do cabeçalho da aba Respostas...');
    
      // Recria o cabeçalho usando Respostas.gs
      if (typeof reconstruirCabecalhoRespostas === 'function') {
        var aba = reconstruirCabecalhoRespostas();
      
        // Lê o cabeçalho atualizado
        var cabecalho = aba.getRange(1, 1, 1, aba.getLastColumn()).getValues()[0];
        var totalColunas = cabecalho.length;
        var totalPerguntas = obterPerguntas().length;
      
        Logger.log('Cabeçalho atualizado com sucesso!');
        Logger.log('Total de colunas: ' + totalColunas);
        Logger.log('Total de perguntas: ' + totalPerguntas);
      
        return {
          ok: true,
          message: 'Cabeçalho da aba Respostas atualizado com sucesso',
        ...
        throw new Error('Função reconstruirCabecalhoRespostas não encontrada. Verifique se Respostas.gs está carregado.');
      }
    } catch (error) {
      Logger.log('Erro ao atualizar cabeçalho: ' + error.message);
      return {
        ok: false,
        error: error.message,
        message: 'Falha ao atualizar o cabeçalho da aba Respostas',
        timestamp: new Date().toISOString()
      };
    }
  } catch (error) {
    Logger.log("Erro em atualizarCabecalhoRespostas: " + error.message);
    throw error;
  }
}

/**
 * Valida que todas as perguntas do questionário estão presentes no cabeçalho
 * da aba "Respostas".
 * 
 * @return {Object} Resultado da validação
 */
function validarCabecalhoRespostas() {
  try {
    try {
      try {
        ...
        perguntas.forEach(function(p) {
          if (cabecalho.indexOf(p.coluna) === -1) {
            faltando.push(p.coluna);
          }
        });
        ...
        var resultado = {
          ...
          perguntasFaltando: faltando,
          timestamp: new Date().toISOString()
        };
    
        if (resultado.ok) {
          resultado.message = 'Todas as ' + perguntas.length + ' perguntas estão presentes no cabeçalho';
        } else {
          resultado.message = 'Faltam ' + faltando.length + ' perguntas no cabeçalho: ' + faltando.join(', ');
        }
        ...
        Logger.log('Erro na validação: ' + error.message);
        return {
          ok: false,
          error: error.message,
          message: 'Falha ao validar o cabeçalho',
          timestamp: new Date().toISOString()
        };
      }
    } catch (error) {
      Logger.log("Erro em validarCabecalhoRespostas: " + error.message);
      throw error;
    }
  } catch (error) {
    Logger.log("Erro em validarCabecalhoRespostas: " + error.message);
