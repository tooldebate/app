...
function getBackendMaturitySpreadsheet_() {
  // Prefere a planilha canônica do projeto (funciona no web app, onde
  // getActiveSpreadsheet() é null); cai para a ativa quando rodando no editor.
  try {
    if (typeof getProjectSpreadsheet === 'function') {
      var ss = getProjectSpreadsheet();
      if (ss) return ss;
    }
  } catch (error) {}
