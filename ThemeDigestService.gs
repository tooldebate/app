 ...
 * ============================================================================
 * PROJETO: Tool Debate - Um conto por aluno
 ...
 *   - Propriedade de script GEMINI_MODEL (obrigatória para síntese Gemini).
...
var THEME_NEWS_CFG = {
  THEME: 'Escrita criativa e contos na escola',
  QUERY: '"escrita criativa" OR "contos infantis" educacao',
  AUDIENCE: 'criancas do ensino fundamental que escrevem contos autorais que viram fanzines',
  ...
  try {
    try {
      ...
      var data = {
        ...
        model: synthesis.model || synthesis.source,
  ...
  try {
    try {
      ...
      if (apiKey && items.length > 0) {
        var generated = callThemeNewsGemini_(apiKey, buildThemeNewsPrompt_(items));
        if (generated && generated.text) {
          return { text: generated.text, source: 'gemini', model: generated.model };
        }
      }
      return { text: localThemeNewsSynthesis_(items), source: 'local', model: 'local' };
    } catch (error) {
...
function buildThemeNewsPrompt_(items) {
  try {
    var built = PromptContextBuilder.build('themeDigest', {
      theme: THEME_NEWS_CFG.THEME,
      query: THEME_NEWS_CFG.QUERY,
      audience: THEME_NEWS_CFG.AUDIENCE,
      headlines: items
    });
    PromptContextBuilder.logAudit('themeDigest', built.droppedKeys);
    var ctx = built.context;
    var safeItems = Array.isArray(ctx.headlines) ? ctx.headlines : [];
    var headlines = safeItems.map(function(item, index) {
 ...
 * @return {?{text: string, model: string}} Texto gerado ou null.
 */
function callThemeNewsGemini_(apiKey, prompt) {
  var model;
  try {
    model = GeminiModelConfig.getModel();
  } catch (error) {
    Logger.log('ThemeDigest: GEMINI_MODEL indisponivel; fallback local: ' + error.message);
    return null;
  }
