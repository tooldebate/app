...
var ConsentService = (function () {
  ...
  function _hash(subjectId) {
    try {
      ...
      if (!s) throw new Error('Identificador de consentimento ausente.');
  ...
  function grant(subjectId, purpose, opts) {
    ...
    if (responsible.length < 3 || responsible.length > 120) {
      throw new Error('[ConsentService] Responsável deve ter entre 3 e 120 caracteres.');
    }
    var sh         = _hash(subjectId);
    var now        = new Date();
    var days       = Number(opts.validDays || 365);
    if (!isFinite(days) || days < 1 || days > 365) {
      throw new Error('[ConsentService] Validade deve estar entre 1 e 365 dias.');
    }
    ...
    try {
      ...
      for (var i = data.length - 1; i >= 1; i--) {
        if (String(data[i][1]) === sh && String(data[i][2]) === purpose && data[i][6] === 'active') {
          ...
          sheet.getRange(i + 1, 9).setValue(opts.reason || '');
          persisted = true;
          break;
        }
      }
      ...
      if (!persisted) throw new Error('[ConsentService] Consentimento ativo não encontrado para revogação.');
